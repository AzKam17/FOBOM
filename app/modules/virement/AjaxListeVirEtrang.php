...
$sWhere = "";
if ($_GET['sSearch'] != "") {
    $sWhere = "WHERE f.compte_banque <> 'NULL' AND f.nationalite != 'CIV' AND (b.etat = 0 OR b.etat = 2) AND (" .
    ...
    $sWhere = "WHERE f.compte_banque <> 'NULL' AND f.nationalite != 'CIV' AND (b.etat = 0 OR b.etat = 2)"; 
}

$sQuery = "
          SELECT SQL_CALC_FOUND_ROWS SUM(b.montapayer) as cumul, b.fbo_code_distrib, f.nom_complet, f.compte_banque, f.swift_code, f.email, f.ville, f.commune, f.quartier, f.nationalite
...
$sQuery = "     SELECT COUNT(b.fbo_code_distrib),SUM(b.montapayer) 
                FROM bonus as b JOIN fbo as f
                ON b.fbo_code_distrib = f.code_distrib
