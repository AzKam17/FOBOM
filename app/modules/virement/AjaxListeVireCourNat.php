...
$sWhere = "";
if ($_GET['sSearch'] != "") {
    $sWhere = "WHERE f.compte_banque <> 'NULL' AND f.nationalite = 'CIV' AND (t.etat = 0 AND t.typepaie = 2) AND (" .
    ...
    $sWhere = "WHERE f.compte_banque <> 'NULL' AND f.nationalite = 'CIV' AND (t.etat = 0 AND t.typepaie = 2)"; 
}

$sQuery = "
          SELECT SQL_CALC_FOUND_ROWS SUM(t.mtfacture) as cumul, b.fbo_code_distrib, f.nom_complet, f.compte_banque, f.swift_code, f.email, f.ville, f.commune, f.quartier, f.nationalite
...
$sQuery = "     SELECT COUNT(b.fbo_code_distrib),SUM(t.mtfacture) 
                FROM paiement as t JOIN bonus as b JOIN fbo as f
                ON t.bonus_idbonus = b.idbonus AND b.fbo_code_distrib = f.code_distrib
...
while ($aRow = mysqli_fetch_array($rResult)) {
    ...
    $tmp[] = addslashes(utf8_encode(number_format($aRow['cumul'], 0, ',', ' ')));
    $tmp[] = 'Cumul de tous les bonus due';
