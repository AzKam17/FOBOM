...
$sWhere = "";
if ($_GET['sSearch'] != "") {
    $sWhere = "WHERE b.mois = $moisbonu AND b.annee = $annebonu AND f.compte_banque <> 'NULL' AND f.nationalite = 'CIV' AND (t.etat = 0 AND t.typepaie = 2) AND (" .
    ...
    $sWhere = "WHERE b.mois = $moisbonu AND b.annee = $annebonu AND f.compte_banque <> 'NULL' AND f.nationalite = 'CIV' AND (t.etat = 0 AND t.typepaie = 2)"; 
}

$sQuery = "
          SELECT SQL_CALC_FOUND_ROWS t.bonus_idbonus, t.mtfacture, b.mois, b.annee, b.fbo_code_distrib, f.nom_complet, f.compte_banque, f.swift_code, f.email, f.ville, f.commune, f.quartier, f.nationalite
...
$sQuery = "     SELECT COUNT(b.fbo_code_distrib) 
                FROM paiement as t JOIN bonus as b JOIN fbo as f
                ON t.bonus_idbonus = b.idbonus AND b.fbo_code_distrib = f.code_distrib
                WHERE b.mois = $moisbonu AND b.annee = $annebonu AND f.compte_banque <> 'NULL' AND f.nationalite = 'CIV' AND (t.etat = 0 AND t.typepaie = 2) 
                 ";
