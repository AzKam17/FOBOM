...
if ($_SESSION['niveau'] == 1) {
    ...
    if (isset($_POST['an']) AND isset($_POST['mo'])) {//SI les tableau est vide dc on ne peut pas ordonner de virement
        $mois = $_POST['mo'];
        $annee = $_POST['an'];
        $listvir = liste_vire_nat_cumul_encour_mens($mois, $annee);
    } else {
        $listvir = liste_vire_nat_cumul_encour();
    }
    ...
    if (empty($listvir)) {//SI les tableau est vide dc on ne peut pas ordonner de virement
        ...
        $erreurs_d = $succes + $success . ' virement(s) nationaux de bonus ont été validé avec succes !';
        if ($fail !== 0 OR $fails !== 0) {
            $erreurs_b = $fail + $fails . ' virement(s) nationaux de bonus n\'ont pas pu etre validé !';
        }
        include 'modules/virement/virecourchoix.php';
    }
} else {
