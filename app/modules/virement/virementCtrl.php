        ...
        if (!empty($_POST['element_1'])) {
            ...
            if ($type == 1) {// Dc virement national
                if(empty($_POST['datepicker']) OR !isset($_POST['datepicker'])){ //Pas de tri, afficher le cumul
                   include 'modules/virement/virelist.php'; 
                ...
                $moisbonus = $daten[1];
                $anneebonus = $daten[2]; 
                include 'modules/virement/viretrilist.php';
                }
            } else { // Virement etranger
                if(empty($_POST['datepicker']) OR !isset($_POST['datepicker'])){ //Pas de tri, afficher le cumul
                   include 'modules/virement/viretrangelist.php'; 
                } else { // Tri par periode
                //on recupere la date du formulaire pour determiner le tri   
                $post = $_POST['datepicker'];
                $dat = nettoyer($post);
                ...
                $moisbonus = $daten[1];
                $anneebonus = $daten[2]; 
                include 'modules/virement/viretrangetrilist.php';
                }
            }
        } else {
