...
if ($_SESSION['niveau'] == 1) {
    ...
    $listvir = liste_vire_nat_mens($mois, $annee);
    ...
    if (empty($listvir)) {//SI les tableau est vide dc on ne peut pas ordonner de virement
        ...
        $erreurs_d = $succes . ' virement(s) nationaux de bonus ordornné avec succes pour la période du '.$mois.'/'.$annee;
        if ($fail !== 0) {
            $erreurs_b = $fail . ' virement(s) nationaux de bonus n\'ont pas pu etre ordornné pour la période du '.$mois.'/'.$annee;
        }
        include 'modules/virement/virechoix.php';
    }
} else {
