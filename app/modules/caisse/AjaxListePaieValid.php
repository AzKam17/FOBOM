...
$sWhere = "";
if ($_GET['sSearch'] != "") {
    $sWhere = "WHERE p.etat = 1 AND p.typepaie = 1 AND (b.etat = 2 OR b.etat = 1) AND (" .
            ...
            "p.datepaie LIKE '" . mysqli_real_escape_string($gaSql['link'],
                    utf8_decode($_GET['sSearch'])) . "%' OR " .
            "p.numero_recu LIKE '" . mysqli_real_escape_string($gaSql['link'],
                    utf8_decode($_GET['sSearch'])) . "%' OR " .
            "b.annee LIKE '" . mysqli_real_escape_string($gaSql['link'],
                    utf8_decode($_GET['sSearch'])) . "%')";
} else {
   
    $sWhere = "WHERE p.etat = 1 AND p.typepaie = 1 AND (b.etat = 2 OR b.etat = 1)"; 
}

$sQuery = "
          SELECT SQL_CALC_FOUND_ROWS p.idpaiement, p.bonus_idbonus, CONCAT(b.mois,'-',b.annee) as periode, b.montapayer, p.utilisateur_idutilisateur, CONCAT(u.civilite,' ',u.nom,' ',u.prenom) as caissier, p.datepaie, DATE_FORMAT(p.datepaie,'%d/%m/%Y a %Hh%m') as datpaie, p.etat, p.mtfacture, p.charge, p.typepaie, p.idcaissier, p.numero_recu
          FROM paiement as p JOIN bonus as b JOIN utilisateur as u
          ON p.bonus_idbonus = b.idbonus AND p.idcaissier = u.idutilisateur "
        . "$sWhere"
        . " $sOrder $sLimit";
...
$sQuery = "     SELECT COUNT(p.idpaiement) 
                ...
                WHERE p.etat = 1 AND p.typepaie = 1 AND (b.etat = 2 OR b.etat = 1) ";
...
while ($aRow = mysqli_fetch_array($rResult)) {
    ...
    $tmp[] = addslashes(utf8_encode($aRow['caissier']));
    //$tmp[] = addslashes(utf8_encode($aRow['numero_recu']));
    $tmp[] = '<a href="index.php?module=caisse&action=info_paie_valid&id=' . $aRow['idpaiement'] . '" ><button title="detail du paiement" class="btn btn-primary btn-social-icon btn-sm" type="button"><i class="fa fa-arrow-circle-right"></i> </button></a>';
