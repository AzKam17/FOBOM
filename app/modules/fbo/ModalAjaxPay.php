...
function fbo_bonus_detail($code) {
    ...
    $requete = $pdo->prepare
            (" 
               ...
               WHERE b.idbonus = :id AND ( b.etat = 0 OR b.etat = 2 )
						");
...
if (!empty($_GET['id']) AND ! empty($_GET['mt'])) {
    ...
    $bonus_infos = fbo_bonus_detail($idcredit);
    ?>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <p>Vous avez décidé de payer le bonus Numero : <strong><?php echo $idcredit; ?></strong>&nbsp;&nbsp; d'un montant de : <strong><?php echo utf8_encode(number_format($mtcredit, 0, ',', ' ')); ?> F CFA</strong></strong> </p>
            </div>
            <form autocomplete="off" method="post" action="index.php?module=fbo&amp;action=payBonusMens" target="_blank"> 
                        ...
                        <table class="table table-bordered">
                            ...
                            <?php foreach ($bonus_infos as $b) { ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="text" class="form-control" id="noteinput" name="noteinput" value="" placeholder="Montant à payer">
                                    </td>
                                </tr>
                            <?php } ?>
                        </table>
                ...
                <input type="hidden" name="idbon" value="<?php echo $idcredit; ?>" />
                <input type="hidden" name="mtbon" value="<?php echo $mtcredit; ?>" />
                <input type="hidden" name="code" value="<?php echo $code; ?>" />
                <input type="hidden" name="nationalite" value="<?php echo $nation; ?>" />
            </form>
        </div>
        <!-- /.modal-content -->
    </div>
